...
int main(int argc, char *argv[]) {
	...
	if (res == sol_res)
		...
		ex("Risposta errata e percorso non fornito", 0.0);
	...
	for (auto cmd : spostamenti) {
		...
		if (!conn[da][dove])
			ex("Spostamento lungo arco inesistente", 0.0);
		// sposto il corteo
		pos[chi] = dove;
	...
	if (pos[1] != D[1] || pos[2] != D[2])
		ex("Destinazioni non raggiunte", 0.0);

	if (distanza > sol_res)
		ex("Contattare lo staff", 0);
	if (distanza == sol_res)
		ex("Output corretto", 1);

	ex("Percorso subottimale", 0.5f * distanza / sol_res);
}
