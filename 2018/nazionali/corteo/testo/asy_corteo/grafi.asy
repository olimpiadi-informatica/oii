...
real max_bend  = .6; // massimo bending per un arco
...
// disegno un nodo
void node(pair pos, real k, string txt = "", pen color = black, pen background = white) {
	path c = circle(pos, node_size*k);
	fill(c, background);
	draw(c, black+1);
	label(scale(text_size*k)*txt, pos, color);
}
...
void edge(pair start, pair end, string txt = "", pen p = black, real deg = 0, real bending = 0, bool arrow = true) {
	...
	if (start != end) {
		...
		pair vec = (-d.y, d.x);
		l = (start+d*0.2) .. (mid+vec*bending) .. (end-d*0.2);
	} else {
...
void main(pair[] P, int[][] E, int[] colE) {
	// calcolo parametri
	calcdeg(P, E);
	real ymax = 0;
	...
	for (int i=0; i<P.length; ++i) {
		real k = 0.8+0.2;
		node(P[i], k, string(i), black, white);
	}
}
