...
void griglia(int N, int M) {
	draw((0,0) -- (M*GRID_SIZE,0));
	draw((M*GRID_SIZE,0) -- (M*GRID_SIZE,N*GRID_SIZE));
	draw((M*GRID_SIZE,N*GRID_SIZE) -- (0,N*GRID_SIZE));
	draw((0,N*GRID_SIZE) -- (0,0));
	for (int i = 0; i < M-1; i+=1) {
		draw( ((i+1)*GRID_SIZE, 0) -- ((i+1)*GRID_SIZE, N*GRID_SIZE), dotted);
	}
	for (int i = 0; i < N-1; i+=1) {
		draw( (0,(i+1)*GRID_SIZE) -- (M*GRID_SIZE,(i+1)*GRID_SIZE), dotted);
	}
}
