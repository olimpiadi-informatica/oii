...
pen bluish = rgb("0084FF");

draw_plane(1, "$1$", "plane_cut_red.eps");
draw_interval(1, 1, 5, ita_red);
select_tick(1, 2, ita_red);

draw_plane(2, "$2$", "plane_cut_white.eps");
// Custom code:
//   draw_interval(2, 1, 9, white);
// does not work in this case.
draw((1 - .5, -2) -- (9 + .5, -2), black + linewidth(2.3));
draw((1 - .5, -2) -- (9 + .5, -2), white + linewidth(1.5));
for (int i = 1; i <= 9; ++i) {
	fill(circle((i, -2), .15), black);
	fill(circle((i, -2), .13), white);
}
fill(circle((5, -2), .10), black);

draw_plane(3, "$3$", "plane_cut_green.eps");
draw_interval(3, 6, 8, ita_green);
select_tick(3, 8, ita_green);

draw_plane(4, "$4$", "plane_cut_blue.eps");
draw_interval(4, 8, 13, bluish);
select_tick(4, 13, bluish);