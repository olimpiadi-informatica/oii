...
line = 1
err = 'label("errore sintattico in riga %s", (0,0), red);'
for row in stdin.readlines():
    try:
        ...
        if var[-1] in cpos:
            err = 'label("componente duplicato in riga %s", (0,0), red);'
            exit(0)
        if var[0] == "in" or not validarg(a) or not validarg(b):
            err = 'label("componente non valido in riga %s", (0,0), red);'
            exit(0)
        cpos[var[-1]] = len(comp)
...
def levelize(c):
    ...
    clev[level[i]] += 1
    if level[i] >= 100:
        print('label("la configurazione eccede il limite di profondità", (0,0), red);')
        exit(0)
