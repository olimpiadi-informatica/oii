...
for (int i=1; i<=4; ++i) {
	draw((-1+.2, i-.02) -- (9-.2, i-.02), dashed + gray(.8));
	label("\small$" + (string)i + "$", (-1.15, i-.02), gray(.6));
}

pen penna = gray(.2);
make_tree((0,0), 3, 4, 6, penna);
make_tree((1,0), 2, 4, 6, penna);
make_tree((2,0), 2, 4, 5, penna);
make_tree((3,0), 1, 2, 2, penna);
make_tree((4,0), 4, 6, 8, penna);
make_tree((5,0), 2, 5, 7, penna);
make_tree((6,0), 3, 5, 5, penna);
make_tree((7,0), 2, 3, 3, penna);
make_tree((8,0), 3, 4, 4, penna);

draw((-1+.2, -0.05) -- (9-.2,-0.05), linewidth(2.5));
...
label(scale(.65) * "(i valori rappresentano le altezze in decametri)", (4,-1.5), S);