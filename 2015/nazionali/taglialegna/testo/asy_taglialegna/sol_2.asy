...
//fill(roundedpath(rettangolo((-5.5,-4.6),18.4,3.85), 0.2), black);
fill(roundedpath(rettangolo((-4.7,-3.6),17.6,1.85), 0.2), white);

int h[] = {2,1,3,1,4,1,1,2,3,2,1,1,2};
int abb[] = {2,2,4,4,-1,-1,7,8,9,-1,-1,12,-1};
for (int i = 0; i < h.length; ++i) {
	...
	if (abb[i] > -1)
		label(scale(.9) * ("$" + (string)(abb[i]) + "$"), (i, -1-1.8));
	else
		label(scale(.9) * ("$\infty$"), (i, -1-1.8));
}
fancy_grid((-.5, -1.5), h.length, 1);
fancy_grid((-.5, -1.5-1.8), h.length, 1);
label("Altezza", (-1,-1.0), align=LeftSide);
label(scale(.8) * "\texttt{abbattitore}", (-1,-1.0 - 1.8), align=LeftSide);
