...
for (int i=1; i<=4; ++i) {
	draw((-1-1.5+.2, i-.02) -- (7+.5-.2, i-.02), dashed + gray(.8));
	label("\small$" + (string)i + "$", (-1.15-1.5, i-.02), gray(.6));
}

pen penna = gray(.8);
make_tree((0,0), 2, 4, 4, penna, 90);
make_tree((1,0), 3, 5, 6, penna, 90);
make_tree((2,0), 2, 4, 5, penna, 90);
make_tree((3,0), 1, 2, 2, penna, 90);
make_tree((4,0), 4, 6, 8, penna, 90);
make_tree((5,0), 2, 5, 7, gray(.2));
make_tree((6,0), 1, 3, 3, gray(.5));

draw((-1-1.5+.2, -0.05) -- (7+.5-.2,-0.05), linewidth(2.5));
...
for (int i = 0; i < h.length; ++i) {
	...
	if (i <= 4) {
		...
		if (i == 5)
			fill((i-.5,hh) -- (i+.5,hh) -- (i+.5, hh+1) -- (i-.5, hh+1) -- cycle, gray(.7));
		else
			fill((i-.5,hh) -- (i+.5,hh) -- (i+.5, hh+1) -- (i-.5, hh+1) -- cycle, gray(.9));
	}
	label("$" + (string)h[i] + "$", (i, -1));
}
grid((-.5, -1.5), 7, 1, black + linewidth(.8));

label(rotate(-20) * "$\rightarrow$", (5.1,1.6), E);