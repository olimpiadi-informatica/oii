...
edge(nodes[3], nodes[0], 2, 2);
edge(nodes[0], nodes[2], 4, 2);
edge(nodes[2], nodes[3], 1, 2);
edge(nodes[4], nodes[3], 3, 2);


string dist[] = {"$1$", "$0$", "$2$", "$2$", "$1$"};
string velo[] = {"$3$", "$\infty$", "$3$", "$2$", "$5$"};
pen pens[] = {black, black, black, black, black};
...
for (int i = 0; i < nodes.length; ++i) {
    ...
    if (i != 3)
        ...
        fill(r, rgb(256/256, 256/256, 200/256));
    draw(r, gray(.5));
}
draw(rect(s, nodes.length * .7, .6));
...
for (int i = 0; i < nodes.length; ++i) {
    ...
    else if (i == Luca && (active[i] == 0))
        node(nodes[i], bord = gray(.4), "$" + (string)(i+1) + "$", inner = rgb(256/256, 200/256, 200/256), lbl = gray(.4));
    else if (i == Luca && active[i] == 3)
        node(nodes[i], bord = black, "$" + (string)(i+1) + "$", inner = rgb(256/256, 200/256, 200/256), lbl = black);
