...
// disegno un nodo
void node(picture pic, pair pos, string txt = "", pen color = black, pen background = white) {
	path c = circle(pos, node_size);
	fill(pic, c, background);
	draw(pic, c, black+1);
	label(pic, scale(text_size)*txt, pos, color);
}

// disegno un arco
void edge(picture pic, pair start, pair end, string txt = "", pen p = black, real deg = 0, real bending = 0, bool arrow = true) {
    path l;
    if (start != end) {
    ...
    if (arrow) draw(pic, l, p, EndArrow(21*node_size));
    else draw(pic, l, p);
    label(pic, scale(text_size)*txt, l, p);
}

// interfaccia per disegnare un arco che usa parametri ottimali calcolati
void ed(picture pic, pair[] P, int[][] E, int i, string txt = "", pen p = black, bool arrow = orientato) {
    edge(pic, P[E[i][0]], P[E[i][1]], txt, p, deg[E[i][0]], bend[i], arrow);
}
...
void main(picture p, pair[] P, pen[] colV, int[][] E, pen[] colE, bool[] orE, int[] A, string[] lbl) {
    // calcolo parametri
    ...
    for (int i=0; i<E.length; ++i)
        ed(p, P, E, i, colE[i], orE[i]);
    for (int i=0; i<P.length; ++i) {
        node(p, P[i], lbl[i], black, colV[i]);
        fill(p, circle(P[i] + NW*node_size, node_size*0.5), heavygrey);
        label(p, scale(0.9)*string(A[i]), P[i] + NW*node_size, white);
    }
}
