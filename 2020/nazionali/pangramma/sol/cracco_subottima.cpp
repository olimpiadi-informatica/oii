...
map<pair<ull, ull>, ull> memoiz;
ull raise(ull x, ull y) {

  if (memoiz[make_pair(x, y)]) return memoiz[make_pair(x, y)];
  if (y && memoiz[make_pair(x, y-1)]) return memoiz[make_pair(x, y)] = (memoiz[make_pair(x, y-1)] * x) % MOD;

  // ull sol = raise_fastexp(x, y);
  ull sol = 1;

  for (ull i = 0; i < y; i++)
    sol = (sol * x) % MOD;

  return memoiz[make_pair(x, y)] = sol;
}
...
void checkk(int K) {
    ...
    cout << get() << " and " << rescmp << endl;
    assert(get() == rescmp);
}

int conta(int N, int K, std::vector<int>& V) {

  MAP_SIZE = min((int)V.size() * 2, MAX_MAP_SIZE);
  ...
  for (int i = 0; i < V.size(); i++) {
    ...
    if (i >= MLEN-1) {
      ...
      // cout << "Divide2 " << F[V[i-(MLEN-1)]] << endl;
      divide(F[V[i-(MLEN-1)]]);
      F[V[i-(MLEN-1)]]--;
